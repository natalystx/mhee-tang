...
const uploadDocument = async (params: UploadDocumentParams) => {
  try {
    const { ownerId, folderName, content, ext, mimeType, name } = params;

    const fileNamed = `${folderName}/${name}.${ext}`;
...
const listDocumentsWithPrefixAsBuffer = async (prefix: string) => {
  try {
    const files = await documentRepo.listFilesWithPrefixAsBuffer(prefix);
    return files;
  } catch (error) {
    throw error;
  }
